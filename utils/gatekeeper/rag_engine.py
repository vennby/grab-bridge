...
def _to_serializable(obj):
    """
    Convert numpy arrays/scalars into plain Python objects.
    """
...
def build_repo_index(repo_url: str) -> Dict[str, str]:
    _ensure_cache_dir()
...
def _load_function_index() -> List[Dict]:
    """Load the pre-built grab function index if it exists."""
...
def _load_index(repo_url: str) -> Dict[str, object]:
    ...
    # Try to load repo-specific index first
    ...
    # Fallback to function index if repo-specific index doesn't exist
    functions = _load_function_index()
    
    if functions:
        # Convert function entries to chunk format
        chunks = []
        for idx, func in enumerate(functions):
            # Filter by repo if possible
            if func.get("repo") == repo:
                chunks.append({
                    "id": f"{func.get('repo')}:{func.get('file')}:{func.get('function')}:{idx}",
                    "path": func.get("file", "unknown"),
                    "text": f"Function: {func.get('signature')}\n\nContext: {func.get('context', '')}",
                    "function": func.get("function"),
                    "language": func.get("language"),
                    "embedding": [],  # No embedding for function index
                    "norm": 0.0,
                })
    ...
    raise FileNotFoundError(
        "Index not found. Run /api/rag/index to build one, or ensure grab_function_index.jsonl exists."
    )
...
def ask_repo(
    ...
    # Only compute embeddings if using embedding-based index
    query_embedding = None
    if index["meta"].get("source") != "function_index":
    ...
    for chunk in chunks:
        # Use different scoring based on index type
        if query_embedding and chunk.get("embedding"):
            ...
            # For function index, use only keyword scoring
